...
| `WidgetRenderer*.cs`, `MetricsSnapshot.cs` | het widget tekenen naar een gewone `Bitmap` zonder venster (`WidgetRenderer.Render(ctx, breedte, dpi)` met een `WidgetRenderContext`: momentopname, historie, schijven, ping); `WidgetForm` bewaart alleen venster, invoer, tooltip en menu en roept de renderer aan. Idem `DashboardRenderer` en `FullscreenRenderer`. Alle drie lezen één `MetricsSnapshot` (onveranderlijke momentopname van de sampler, `Metrics.Current`). Renderers zijn te testen zonder venster (zie `tests`); `GdiCache.ThreadCheck` staat in tests uit, tests draaien na elkaar |
...
| `DashboardForm.cs`, `DashboardRenderer.cs`, `DashboardModel.cs` | bureaublad-dashboard: `DashboardForm` = venster (layered window, z-orde, timers, muis); `DashboardRenderer` = tekenen naar een gewone `Bitmap` zonder venster (indeling/masonry, tegels; ontvangt `MetricsSnapshot`, schijven, DPI expliciet; `Measure` geeft alleen de maat; testbaar); `DashboardModel` = `Ring`, `MetricHistory`, `DashContext` |
| `FullscreenForm.cs`, `FullscreenRenderer*.cs`, `FullscreenView.cs` | fullscreen cockpit: vast canvas 1920×1080 dat meeschaalt; overzicht + detail per tegel; Esc = terug/sluiten. `FullscreenForm` = venster, invoer, navigatie, tour, timers; `FullscreenRenderer` (partial: .cs hulpen, .Header, .Overview, .Detail, .Specs, .Sensors) = alleen tekenen: `Paint(g, size, FullscreenView, MetricsSnapshot, background?)`, `Hits`/`HitAt`, `Scale`; `FullscreenView` = weergavetoestand (pagina, venster, scroll, hover, tour); klok/tijd en specificatiebron zijn injecteerbaar (tests: `FullscreenRendererTests`) |
