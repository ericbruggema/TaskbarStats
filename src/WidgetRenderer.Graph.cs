...
// Bron: de geschiedenis uit de tekencontext (_history 1 Hz, temperatuur eigen ringen); de reeks sluit tijdens de spaarstand gewoon aan.
internal sealed partial class WidgetRenderer
{
    ...
    private int DrawTempGraph(Graphics g, int x, string what, double t)
    {
        ...
        return DrawGraphRing(g, x, label, what == "CPU" ? "cputemp" : "gputemp", t, what == "CPU" ? _ctx.CpuTempHistory : _ctx.GpuTempHistory);
    }
    ...
    private double[] PingSamples()
    {
        ...
        if (now - _pingCacheAt >= 900) { _pingCacheAt = now; _pingCache = (_ctx.PingSamples?.Invoke() ?? Array.Empty<double>()); }
        return _pingCache;
    }
